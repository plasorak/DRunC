...
def process_manager_shell(ctx, process_manager_address:str, log_level:str) -> None:
    ...
    desc = None
    process_manager_shell_log.debug(f"[green]{getpass.getuser()}[/green] connecting to [green]process_manager[/green] at address [green]{process_manager_address}[/green]")
    try:
    ...
    log.warning(f"[green]{getpass.getuser()}[/green] connected to the process manager through a [green]drunc-process-manager-shell[/green] via address [green]{process_manager_address}[/green]")
    ...
    def cleanup():
        ctx.obj.terminate()
        log.warning(f"[green]{getpass.getuser()}[/green] disconnected from the process manager through a [green]drunc-process-manager-shell[/green]")
